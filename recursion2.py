...
#  Description: CodingBat recursion problem set
...
#  Date Last Modified: 10 Mar 2021
...
def groupSum(start, nums, target):
    if target == 0: # Base case
        return True
    elif start == len(nums): # Impossible
        return False
    elif groupSum(start + 1, nums, target - nums[start]):
        return True
    return groupSum(start + 1, nums, target) # Use start to keep track
...
def groupNoAdj(start, nums, target):
    if target == 0:
        ...
        return False
    # This time it is +2 instead of +1 to avoid adjacent numbers
    elif groupNoAdj(start + 2, nums, target - nums[start]):
        return True
    # Don't have to worry about adjacence if the previous start failed anyway
...
def groupSum5(start, nums, target):
    if start >= len(nums):
        if target == 0:
            ...
            return False
    elif nums[start] % 5 == 0: # i.e. if is_multiple_5()
        # Check condition of next number following multiple == 1
...
def splitArrayHelper(nums, i, balance): # To determine if sides are indeed equal
    if i == len(nums):
        return balance == 0
    elif splitArrayHelper(nums, i + 1, balance + nums[i]):
        return True
    return splitArrayHelper(nums, i + 1, balance - nums[i])
...
def splitOdd10Helper(nums, i, group1, group2): # To determine if sides are indeed odd
    if i == len(nums):
        return (group1 % 2 == 1 and group2 % 10 == 0) or \
               (group2 % 2 == 1 and group1 % 10 == 0)
    elif splitOdd10Helper(nums, i + 1, group1 + nums[i], group2):
        return True
    return splitOdd10Helper(nums, i + 1, group1, group2 + nums[i]) # Recursive action
...
def split53Helper(nums, i, balance): # To determine if sides are indeed equal
    if i == len(nums):
        return balance == 0
    elif nums[i] % 5 == 0:
        ...
        return split53Helper(nums, i + 1, balance - nums[i])
    elif split53Helper(nums, i + 1, balance + nums[i]):
        return True
    return split53Helper(nums, i + 1, balance - nums[i]) # Recursive action
